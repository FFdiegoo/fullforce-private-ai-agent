    ...
    if (error.message.includes('match_documents') || error.message.includes('vector')) {
      try {
        ...
        const fallbackSystemPrompt = mode === 'technical'
          ? buildCeesSystemPrompt({
              retrievalWarning: 'De automatische documentzoekactie is mislukt. Geef duidelijk aan dat je zonder actuele context werkt en vraag om ontbrekende gegevens voordat je adviseert.',
            })
